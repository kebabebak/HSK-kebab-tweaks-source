...
namespace HSK.KebabTweaks
{
        ...
        public static void Apply(Harmony harmony)
        {
            try
            {
                ...
                Log.Message(
                    ...
                    $"{(KebabTweaksSettings.TradeCaravanLordEnableLogging ? "ON" : "OFF")}). " +
    ...
    internal static class TradeDismissTrigger_Patch
    {
        ...
        /// <summary>
        /// Resolves the CreateGraph dismiss Trigger_Custom predicate (compiler-generated lambda).
        /// RimWorld 1.6 renamed &lt;CreateGraph&gt;b__0 → b__1; scan nested DisplayClass methods
        /// that read traderDismissed when the fixed name is absent.
        ///
        /// Находит dismiss-предикат CreateGraph (compiler-generated lambda). В RW 1.6
        /// &lt;CreateGraph&gt;b__0 стал b__1; при отсутствии фиксированного имени сканирует nested
        /// DisplayClass-методы с traderDismissed.
        /// </summary>
        public static MethodBase FindTargetMethod()
        {
            if (DisplayClassType == null)
            {
                return FindDismissTriggerInNestedTypes();
            }
            ...
            {
                MethodInfo method = AccessTools.Method(DisplayClassType, methodName);
                if (method != null && MethodReadsTraderDismissed(method))
                {
                    return method;
                }
            }
        ...
        private static MethodBase FindDismissTriggerInNestedTypes()
        {
            foreach (Type nested in typeof(LordJob_TradeWithColony).GetNestedTypes(BindingFlags.NonPublic))
            {
                if (nested?.Name == null || nested.Name.IndexOf("DisplayClass", StringComparison.Ordinal) < 0)
                {
                    continue;
                }

                foreach (MethodInfo method in nested.GetMethods(BindingFlags.Instance | BindingFlags.Public |
                                                                BindingFlags.NonPublic))
                {
                    if (method == null || method.Name == null || method.Name.IndexOf("CreateGraph", StringComparison.Ordinal) < 0)
                    {
                        continue;
                    }

                    if (MethodReadsTraderDismissed(method))
                    {
                        return method;
                    }
                }
        ...
        private static bool MethodReadsTraderDismissed(MethodInfo method)
        {
            ...
            for (int i = 0; i < il.Length - 4; i++)
            {
                ...
                try
                {
                    ...
                    if (field != null && field.Name == "traderDismissed")
                    {
                        return true;
                    }
                }
        ...
        {
            if (!KebabTweaksSettings.EnableTradeCaravanLordFix)
            {
                return true;
            }
            ...
            if (IsUsableTrader(trader))
            {
                ...
                {
                    __result = true;
                    TradeCaravanLordFixPatchLog.Message(
    ...
    internal static class GetTraderCaravanRole_Patch
    {
        ...
        public static void Postfix(Pawn p, ref TraderCaravanRole __result)
        {
            ...
            if (KebabTweaksSettings.TradeCaravanLordEnableLogging &&
                p != null &&
    ...
    internal static class TradeDiagPatches
    {
        public static void Apply(Harmony harmony)
        {
            ...
            TradeCaravanLordFixPatchLog.Message("[TradeCaravanLordFixPatch] Diagnostic patches registered.");
        }
    }
    ...
    internal static class Diag_GenerateTrader
    {
        ...
        public static void Prefix(PawnGroupMakerParms parms, PawnGroupMaker groupMaker, TraderKindDef traderKind)
        {
            ...
            if (!KebabTweaksSettings.EnableTradeCaravanLordFix || !KebabTweaksSettings.TradeCaravanLordEnableLogging)
            {
                return;
            }
            ...
            TradeCaravanLordFixPatchLog.Message(sb.ToString().TrimEnd());
        }
        ...
        public static void Postfix(Pawn __result)
        {
            ...
            {
                if (KebabTweaksSettings.EnableTradeCaravanLordFix &&
                    KebabTweaksSettings.TradeCaravanLordEnableLogging)
                {
                    TradeCaravanLordFixPatchLog.Message(
    ...
    internal static class Diag_GeneratePawn
    {
        ...
        public static void Postfix(PawnGenerationRequest request, Pawn __result, PawnKindDef __state)
        {
            ...
            TradeCaravanLordFixPatchLog.Message(sb.ToString());
    ...
    /// <summary>
    /// Writes Log.Message only when TradeCaravanLordEnableLogging is on.
    ///
    /// Пишет Log.Message только при включённом TradeCaravanLordEnableLogging.
    /// </summary>
    public static class TradeCaravanLordFixPatchLog
    {
